...
pipeline {
    ...
    environment {
        REPO_NAME = 'botnoi-platform-tool'
        REPO_URL = 'registry.gitlab.com/botnoi-text2speech-v2/botnoi-platform-tool'
    ...
    stages {
        ...
        stage('OWASP Dependency-Check Scan') {
            when { anyOf { branch 'staging'; tag 'release-*' } }
            steps {
                    dependencyCheck additionalArguments: '--scan ./ --disableYarnAudit --disableNodeAudit --disableAssembly --noupdate', odcInstallation: 'DP-Check'
        ...
        stage('Docker Build Staging') {
            when { branch 'staging' }
            steps {
                sh 'export GOPROXY=https://proxy.golang.org/cached-only'
                sh "docker build -t ${REPO_NAME} -f ./Dockerfile.staging ."
        ...
        stage('Docker Build Tag') {
            when { tag "release-*" }
            steps {
                sh "docker build -t ${REPO_NAME} -f ./Dockerfile.prod ."
                sh "docker tag ${REPO_NAME} ${REPO_URL}:${TAG}" 
            }
        }
